defmodule Farmbot.BotState.Transport.AMQP.ConnectionWorker do
  @moduledoc "Caches a single AMQP connection."

  use GenServer
  use Farmbot.Logger
  require Logger
  import Farmbot.System.ConfigStorage, only: [update_config_value: 4]
  ...
  def terminate(_, conn) do
    if Process.alive?(conn.pid) do
      try do
        Logger.debug 3, "Closing AMQP connection."
  ...
  def handle_info({:DOWN, _, :process, _pid, reason}, conn) do
    ...
    if reason not in ok_reasons do
      Logger.error 1, "AMQP Connection closed: #{inspect reason}"
